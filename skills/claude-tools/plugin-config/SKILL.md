...
```bash
python3 "$SKILL_DIR/scripts/diagnose.py" <plugin>@<marketplace>
```
...
python3 "$SKILL_DIR/scripts/set_config.py" \
    <plugin>@<marketplace> \
...
python3 "$SKILL_DIR/scripts/bulk_config.py" \
    --from-sops ~/code/<repo>/secrets \
