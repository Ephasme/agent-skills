...
A fixed set of code smells (Fowler, *Refactoring*, ch. 3) that the **Standards** axis carries on
top of whatever the repo documents — so a repo that documents nothing still gets a real review.
...
- **Always a judgement call.** Each entry is a labelled heuristic — "possible Feature Envy" — never
  a hard violation. And like any standard here, skip anything tooling already enforces.

Each reads *what it is* → *how to fix*. Match them against the diff, not against the codebase at
large: the question is what **this change** introduced.
