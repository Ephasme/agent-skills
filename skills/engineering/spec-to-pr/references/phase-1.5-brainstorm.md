...
the plan (Phase 2) is where a design gets locked into an ordered task list, and Phase 3 then
hardens *that* plan. Neither reconsiders the approach; they sharpen the one you brought. So a
...
failure mode this pipeline is *worst* at catching — hardening assumes the design is
sound and attacks the details. A few minutes of divergence here is cheap insurance; a re-plan
after Phase 4 discovers the approach was wrong is not.

## Running it

Lay out **2–3 candidate approaches**, and for each name the trade-offs, the risk it carries, and
what it costs to reverse. Then pick one and write down *why* — the reason is what Phase 2 plans
against and what a reviewer can later check.
...
an edit to the tree. A "quick spike to see if it works" is still writing code before Phase 4, and
it's forbidden here like everywhere in 0–3. If you need evidence that an approach is feasible,
...
`✅ Phase 1.5 (BRAINSTORM) — chose per-tenant token bucket over per-key (shared budget matches the AC), 2 approaches weighed`
