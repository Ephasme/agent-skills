...
and the one Phase 4 hands task by task to its per-task build loop — so its job is to be **specific
...
**Every plan starts with this header** — the Global Constraints block is load-bearing: Phase 4's
...
Each task is its own `### Task N` section — Phase 4's `task-brief` extracts tasks by that heading,
...
Beyond the structure above, the plan is only usable by Phase 4 if each task carries:
...
- **Risks** — what could go wrong, what's uncertain, what you're assuming. Phase 3 hunts here
...
The plan must be a **file**, not a message — Phase 3 edits it in place across its
loop-until-clean rounds, and Phase 4 reads it. Put it wherever the repo keeps plans (check for
