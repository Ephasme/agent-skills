...
Phases **0–3 are a code freeze**. The freeze **lifts at Phase 4 (IMPLEMENT)** and stays lifted
for Phases 4–8, where writing code is the job. Before Phase 4, the deliverable of *every*
phase is a **document**, never a diff.

## What "writing code" means (all of this is forbidden in Phases 0–3)
...
## What you MAY write in Phases 0–3

Exactly two things. Nothing else in the repository may change.

1. **The plan file** (Phase 2; amended in place by Phase 3).
2. **Scratch notes outside the repository** (the session scratchpad).

Creating the feature branch is allowed (Phase 4 pre-flight) — it carries no content.
...
| "The builder will need this file anyway." | Then it belongs in the plan, and the Phase 4 implementer writes it, and a reviewer reviews it. |
...
| "My prompt said to review the code, so I fixed it." | A review returns findings. If a prompt seems to ask a pre-Phase-4 agent for a diff, that prompt is wrong; return the finding instead. |

## If you believe you must write code before Phase 4
...
to the human). A change discovered during Phases 1–3 is *information*, and the pipeline is
built to consume it: it becomes a plan amendment, then a task an implementer picks up in
Phase 4, then a reviewer's diff. Every one of those steps is skipped by an early edit.

## Enforcement (orchestrator)

- **Paste the block below, verbatim, into every subagent prompt dispatched in Phases 0–3** —
  explorers and hardening agents. Any of them can hold an edit
  tool; assume none of them knows this rule unless you tell it.
- **Prefer read-only agents.** Use `Explore` for exploration when it fits — a tool the agent
  doesn't have is a rule it cannot break.
- **Check the tree before Phase 4 launches.** `git status --porcelain` must show nothing but
  the plan file. Report the check in the Phase 4 receipt.
- **If an agent broke the freeze:** revert its edits (`git restore`/`git checkout --` the
  ...
  plan amendment. Never carry the edit into Phase 4's base commit — an unreviewed diff that
  predates implementation is exactly what this rule exists to prevent.

## The canonical prompt block (copy verbatim into every Phase 0–3 subagent)
...
separate phase, performed by different agents against the reviewed plan. It is not your job
and you must not start it.
...
You MAY write to: the plan file and your own report — whichever of those your task names.
Nothing else in the repository may change.
