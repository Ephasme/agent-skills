...
- **Fallback (no Notion MCP — the common case here):** Notion pages need auth, so you can't
  ...
  can read). Don't guess at a page you can't see. Note the ticket id regardless — Phase 6 needs
...
- **Fallback:** if the GitHub MCP isn't connected, use the **`gh` CLI**
  ...
  to paste the issue.
- Note the `<org>/<repo>` and issue number — Phase 6 links the PR back with `Closes #<n>`.
