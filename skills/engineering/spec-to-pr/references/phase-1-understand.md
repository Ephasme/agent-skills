...
### Explore in parallel when the ticket spans areas

If the ticket touches several distinct areas — an API surface *and* a migration *and* a frontend
consumer — dispatch **one `Explore` agent per area, all in a single message**. They are read-only
and independent, so there is nothing to serialize, and each one's search noise stays in its own
context instead of yours. Give each a narrow brief ("map how tenant quota is enforced today: the
middleware, its config, and its tests") and ask for a short report with `file:line` anchors.

One area, one agent — don't fan out for the sake of it. And `Explore` is read-only, which is the
point during the freeze: a tool the agent doesn't have is a rule it cannot break (Rule Zero).
...
explicitly, mark it as an assumption in the plan, and carry it to the Phase-9 handoff** so it
