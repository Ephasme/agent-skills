...
All paths below are relative to this skill's own directory ([`../SKILL.md`](../SKILL.md)) — the
`scripts/` prefix means the sibling of this `references/` folder.
...
**Launching the server:** start it exactly as above. The script backgrounds the server itself on Unix-like hosts; on Windows-likes (Git Bash, MSYS, Cygwin) it detects the reaping and switches to foreground mode, which blocks the call — so launch it with the agent's own background-execution mechanism there.

If your environment reaps detached processes, pass `--foreground` **and** launch the command with the platform's background-execution mechanism, so the harness owns the backgrounding and the server survives across turns. If it has no such mechanism, keep `--foreground` and accept that the call blocks until the session ends.
